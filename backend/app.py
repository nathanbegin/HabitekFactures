...
def patch_facture(fid):
    ...
    # Détecter le type de contenu pour extraire les données correctement
    ...
    if 'multipart/form-data' in content_type:
        # Cas 1 : La requête est un formulaire, potentiellement avec un fichier
        data = request.form.to_dict()
        file = request.files.get('fichier')
        remove_file = data.pop('remove_file', 'false').lower() == 'true'
    elif 'application/json' in content_type:
        # Cas 2 : La requête est du JSON pur (pas de fichier)
        data = request.get_json() or {}
        # remove_file peut aussi être envoyé en JSON
        remove_file = data.pop('remove_file', False)
    ...
    try:
        ...
        # Si la seule action était de supprimer un fichier (remove_file=true),
        # il faut s'assurer que le champ chemin_fichier est bien dans la requête
        if remove_file and 'chemin_fichier = %s' not in sets:
             sets.append('chemin_fichier = %s')
             vals.append(None)
...
def remplacer_facture_fichier(facture_id):
    ...
    # 1. Vérification et réception du fichier
    if 'fichier' not in request.files:
        return jsonify({"error": "Aucun fichier fourni dans la requête (attendu sous la clé 'fichier')."}), 400
    
    nouveau_fichier = request.files['fichier']
    if nouveau_fichier.filename == '':
        return jsonify({"error": "Nom de fichier invalide."}), 400

    # 2. Sécuriser le nom du fichier (pour l'enregistrement)
    original_filename = nouveau_fichier.filename
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")
    
    # safe_filename est le nom du fichier sur le disque
    safe_filename = secure_filename(f"{timestamp}_{facture_id}_{original_filename}")
    
    # new_file_path est le chemin complet (uploads/safe_filename)
    upload_folder_config = app.config['UPLOAD_FOLDER']
    new_file_path = os.path.join(upload_folder_config, safe_filename)
    ...
    try:
        ...
        cur.execute("SELECT chemin_fichier FROM public.factures WHERE id = %s", (facture_id,))
        ancienne_facture = cur.fetchone()
        
        if not ancienne_facture:
            return jsonify({"error": f"Facture avec ID {facture_id} introuvable."}), 404
            
        # 4. Enregistrer le nouveau fichier sur le disque
        nouveau_fichier.save(new_file_path)
        
        # 5. Mettre à jour la base de données: on ne met à jour que 'chemin_fichier'
        cur.execute("""
            ...
            WHERE id = %s
        """, (new_file_path, g.user_id, facture_id)) 
        
        # 6. Suppression de l'ancien fichier
        old_path = ancienne_facture['chemin_fichier']
        if old_path:
            # Sécurité: S'assurer que le chemin est bien dans notre dossier d'upload
            if os.path.exists(old_path) and old_path.startswith(upload_folder_config):
                os.remove(old_path)
        
        conn.commit()
        
        # 7. Émettre l'événement SocketIO pour la mise à jour
        socketio.emit("update_facture", {"id": facture_id}) 
        
        # Retourne le nouveau chemin complet qui sera utilisé par le frontend
        ...
        # En cas d'erreur, tentez de supprimer le fichier nouvellement uploadé
...
def create_compte_depense():
    ...
    if date_soumis:
        try:
            datetime.strptime(date_soumis, "%Y-%m-%d")
        except ValueError:
            return jsonify({"error": "date_soumis invalide (AAAA-MM-JJ)"}), 400
...
def list_comptes_depenses():
    ...
    if q:
        where_clauses.append("(cd.id ILIKE %s OR cd.demandeur_prenom ILIKE %s OR cd.demandeur_nom ILIKE %s)")
        params.extend([f"%{q}%", f"%{q}%", f"%{q}%"])
    
    if mode in ("global_ubr", "distinct_ubr"):
        where_clauses.append("cd.mode = %s")
        params.append(mode)
    
    if dfrom:
        try: 
            datetime.strptime(dfrom, "%Y-%m-%d")
            where_clauses.append("cd.date_soumis >= %s")
            params.append(dfrom)
        except ValueError: 
            return jsonify({"error": "'from' invalide (AAAA-MM-JJ)"}), 400
    ...
    if dto:
        try: 
            ...
            return jsonify({"error": "'to' invalide (AAAA-MM-JJ)"}), 400

    if where_clauses:
        query += " WHERE " + " AND ".join(where_clauses)
    
    query += " ORDER BY cd.created_at DESC, cd.id ASC;"

    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    try:
        cur.execute(query, tuple(params)) # Les paramètres sont passés de manière sécurisée
        rows = cur.fetchall()
...
def patch_compte_depense(cid):
    ...
    if "date_soumis" in data and data["date_soumis"]:
        try:
            datetime.strptime(str(data["date_soumis"]), "%Y-%m-%d")
        except (ValueError, TypeError):
            return jsonify({"error": "date_soumis invalide (AAAA-MM-JJ)"}), 400
    if data.get("mode") == "global_ubr" and not data.get("global_ubr"):
        return jsonify({"error": "global_ubr requis lorsque mode=global_ubr"}), 400

    # Construire la requête de manière sécurisée
    allowed_fields = {"mode", "global_ubr", "demandeur_prenom", "demandeur_nom", "date_soumis"}
    fields_to_update = []
    values_to_update = []
    ...
    fields_to_update.append("updated_at = NOW() AT TIME ZONE 'utc'")
    values_to_update.append(cid)
    
    query = f"UPDATE public.comptes_depenses SET {', '.join(fields_to_update)} WHERE id = %s RETURNING *;"

    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    try:
        cur.execute(query, tuple(values_to_update)) # Paramètres passés de manière sécurisée
        row = cur.fetchone()
        if not row:
            return jsonify({"error": "Compte introuvable"}), 404
...
def delete_compte_depense(cid):
    ...
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM public.comptes_depenses WHERE id = %s RETURNING id;", (cid,))
        gone = cur.fetchone()
        if not gone:
            return jsonify({"error": "Compte introuvable"}), 404
...
def detach_facture_from_compte(cid, fid):
    ...
    try:
        ...
        conn.rollback()
        traceback.print_exc()
        return jsonify({"error": "Erreur de détachement", "details": str(e)}), 500
    finally:
        cur.close()
        conn.close()
