        ...
        <section className="card">
          <div className="card-content">
            ...
            <form onSubmit={handleUpload} className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <input type="number" name="annee" defaultValue={ANNEE} placeholder="Année" className="input" required />
              ...
              <input type="file" name="fichier" accept="application/pdf" onChange={e => setFile(e.target.files[0])} className="input" required />
        ...
        <section className="card p-6 relative">
          ...
          <div className="overflow-x-auto">
            <table className="table">
              <thead>
                <tr>
                  <th>#</th><th>Type</th><th>UBR</th><th>Fournisseur</th><th>Montant</th><th>Statut</th><th>…</th>
                </tr>
              </thead>
              <tbody>
                {factures.map((f,i) => (
                  <tr key={f.id} className="hover:bg-gray-50">
                    ...
                    <td>
                      <button onClick={e=>toggleMenu(e,f.id)} className="menu-button">⋮</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {menuOpen.id && (
            <div className="popup-menu" style={{top:menuOpen.y,left:menuOpen.x}}>
              <button onClick={()=>startEditStatus(menuOpen.id,factures.find(f=>f.id===menuOpen.id).statut)} className="block px-4 py-2 hover:bg-gray-100 w-32 text-left">Modifier statut</button>
              <button onClick={()=>deleteFacture(menuOpen.id)} className="block px-4 py-2 hover:bg-gray-100 w-32 text-left text-red-600">Supprimer</button>
            </div>
          )}
        </section>
      </main>
