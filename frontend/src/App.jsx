...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('connect',    () => console.log('SocketIO connected'));
    socket.on('disconnect', () => { console.log('SocketIO disconnected'); setClientCount(0); });
    socket.on('client_count', setClientCount);
  ...
  // --- Authentification ---
  const handleLoginSuccess = (token, loggedUserId, role) => {
    sessionStorage.setItem('habitek_auth_token', token);
    sessionStorage.setItem('habitek_user_role', role);
    sessionStorage.setItem('habitek_user_id', loggedUserId);
    setUserToken(token);
    setUserRole(role);
  ...
  const authorizedFetch = async (url, options = {}) => {
    ...
    if (res.status === 401 || res.status === 403) {
      let errMsg = "Session expirée ou accès refusé.";
