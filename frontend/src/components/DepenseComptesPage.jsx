...
export default function DepenseComptesPage({ authorizedFetch, userRole, API_URL }) {
    ...
    // Formattage d'un montant par devise (fr-CA)
    const formatCurrency = (value, currency = "CAD") =>
        new Intl.NumberFormat("fr-CA", { style: "currency", currency }).format(value);

    // Totaux par devise à partir des factures liées du détail
    const totalsByCurrency = useMemo(() => {
        const totals = {};
        const list = Array.isArray(detail?.factures) ? detail.factures : [];
        for (const f of list) {
            const dev = String(f.devise || "CAD").toUpperCase();
            const val = Number(f.montant);
            if (!isFinite(val)) continue;
            totals[dev] = (totals[dev] || 0) + val;
        }
        return totals;
    }, [detail]);

    // Petits badges “CAD 1 234,56 · USD 120,00”
    const totalBadges = useMemo(() => {
        const entries = Object.entries(totalsByCurrency);
        if (!entries.length) return "0";
        return entries.map(([dev, sum]) => (
            <span
                key={dev}
                className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-slate-100 text-slate-700 mr-2"
            >
                {formatCurrency(sum, dev)} {dev !== "CAD" ? "" : "" /* optionnel */}
            </span>
        ));
    }, [totalsByCurrency]);
                ...
                <div className="mt-6 border rounded p-4">
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-2">
                        ...
                        <div className="sm:col-span-2">
                            <b>Total factures liées :</b> {Array.isArray(totalBadges) ? totalBadges : totalBadges}
                            {/* Optionnel : afficher aussi le nombre de factures */}
                            {/* <span className="ml-2 text-xs text-gray-500">({detail.factures?.length || 0} facture(s))</span> */}
                        </div>
                    </div>
