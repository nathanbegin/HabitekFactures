...
// src/components/MainLayout.jsx
// Structure principale une fois connecté : sidebar + header + contenu + drawer (desktop) / modal (mobile)
...
// Utilitaire : année financière (1er mai → 30 avril)
const getFinancialYear = (date = new Date()) => {
  const y = date.getFullYear();
  return date.getMonth() >= 4 ? String(y) : String(y - 1);
};
...
export default function MainLayout({
  ...
  // ------------------------------
  useEffect(() => {
    const onManage = location.pathname.endsWith("/manage-invoices");
    const canView =
      userRole === "soumetteur" || userRole === "gestionnaire" || userRole === "approbateur";
    if (onManage && canView) {
      fetchFactures(anneeFinanciere);
    ...
    // Sockets → rafraîchir quand new/update/delete_facture
    ...
    if (socket) {
      ...
      socket.on("new_facture", handleNew);
      socket.on("delete_facture", handleDel);
      socket.on("update_facture", handleUpd);
      return () => {
        socket.off("new_facture", handleNew);
        socket.off("delete_facture", handleDel);
        socket.off("update_facture", handleUpd);
      };
    }
  }, [location.pathname, anneeFinanciere, userRole]);

  // ------------------------------
  // API helpers (authorizedFetch)
  // ------------------------------
  async function fetchFactures(year) {
    try {
      const res = await authorizedFetch(`${API_URL}/api/factures?year=${year}`);
      ...
      console.error("fetchFactures error:", e);
      setFactures([]);
    }
  }
  ...
  async function addFacture(formData) {
    try {
      // On passe un FormData directement (laissez authorizedFetch gérer les headers)
      const res = await authorizedFetch(`${API_URL}/api/factures`, {
        method: "POST",
        body: formData,
      });
      if (!res.ok) {
      ...
      // Rafraîchir la liste de l’année courante
      await fetchFactures(anneeFinanciere);
      // Fermer le modal mobile si ouvert
      setFormModalOpen(false);
    } catch (e) {
      console.error("addFacture error:", e);
      alert("Erreur lors de l’ajout de la facture.");
    } finally {
      setUploadProgress(null);
      setTimeLeft("");
  ...
  async function deleteFacture(id) {
    ...
    try {
      const res = await authorizedFetch(`${API_URL}/api/factures/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
      ...
      console.error("deleteFacture error:", e);
    }
  }
  ...
  async function updateFacture(id, patch) {
    try {
      ...
      // On pourrait MAJ localement, on choisit de refetch pour rester simple
      await fetchFactures(anneeFinanciere);
    } catch (e) {
  ...
  async function downloadFactureFile(id, annee) {
    try {
      const res = await authorizedFetch(`${API_URL}/api/factures/${id}/download?year=${annee}`);
      if (!res.ok) {
      ...
      console.error("downloadFactureFile error:", e);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <aside
        ...
        } fixed inset-y-0 left-0 z-40 w-64 bg-white border-r transform transition-transform duration-200 ease-out lg:static lg:translate-x-0`}
      >
        <div className="h-16 border-b flex items-center px-4 gap-3">
          <img src={logo} alt="Habitek" className="h-8 w-auto" />
          <div className="text-sm text-gray-600">Connectés: {clientCount ?? 1}</div>
        </div>
        <nav className="p-3 space-y-1">
          <button
            className="w-full text-left px-3 py-2 rounded hover:bg-gray-100"
            onClick={() => {
              navigate("/dashboard/manage-invoices");
              setIsSidebarOpen(false);
            }}
          >
            Factures
          ...
          {(userRole === "gestionnaire" || userRole === "approbateur") && (
            <button
              className="w-full text-left px-3 py-2 rounded hover:bg-gray-100"
              onClick={() => {
                navigate("/dashboard/manage-budget");
                setIsSidebarOpen(false);
              }}
            >
              Budget
          ...
          <button
            className="w-full text-left px-3 py-2 rounded hover:bg-gray-100"
            onClick={() => {
              navigate("/dashboard/depense-comptes");
              setIsSidebarOpen(false);
            }}
          >
            Comptes de dépenses
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-4">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="truncate font-medium">Tableau de bord</div>
          </div>
          <div className="flex items-center gap-3">
            <select
              ...
              title="Année financière"
            >
              {/* 4 années roulantes */}
              {[0, 1, 2, 3].map((i) => {
                const y = Number(getFinancialYear()) - i;
                return (
        ...
        <div className="flex-1 min-w-0 flex">
          ...
          <main className="flex-1 min-w-0 p-4 lg:p-6 overflow-x-hidden">
            <Routes>
                  ...
                  element={
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {/* Desktop : bouton d’ouverture du drawer */}
                        {canUseDrawer && (
                          <div className="hidden lg:flex items-center gap-2">
                            <button
                              onClick={() => setFormDrawerOpen((v) => !v)}
                        ...
                        {/* Mobile : bouton d’ouverture du modal */}
                        {(userRole === "soumetteur" || userRole === "gestionnaire") && (
                          <button
                            className="lg:hidden px-3 py-1.5 border rounded hover:bg-gray-50"
                            onClick={() => setFormModalOpen(true)}
                          >
                            Ajouter
                          </button>
              ...
              {/* Redirect par défaut */}
              <Route path="*" element={<Navigate to="manage-invoices" replace />} />
            </Routes>
          </main>

          {/* Drawer formulaire (desktop only) */}
          {canUseDrawer && (
            <aside
              className={`hidden lg:flex lg:flex-col lg:shrink-0 border-l bg-white relative transition-[width] duration-150 ease-out ${
                formDrawerOpen ? "" : "overflow-hidden"
              }`}
              style={{ width: formDrawerOpen ? `${drawerWidth}px` : "0px" }}
            >
              {formDrawerOpen && (
      ...
      {/* Modal mobile : ajout facture */}
      {(userRole === "soumetteur" || userRole === "gestionnaire") && formModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40">
          <div className="bg-white w-full sm:max-w-lg sm:rounded-lg sm:shadow-xl sm:mx-4">
            <div className="px-4 py-3 border-b flex items-center justify-between">
              <div className="font-medium">Ajouter une facture</div>
              <button
                onClick={() => setFormModalOpen(false)}
