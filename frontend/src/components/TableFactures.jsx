...
// // Permet de trier les factures par colonne, de télécharger les fichiers associés, de supprimer des factures et de modifier leur statut,
...
//   // Gestion des États
//   // -----------------------------------
//   // État pour la colonne de tri actuelle (par défaut 'date_facture' ou 'id')
...
//       // Si on clique sur la colonne déjà triée, inverser la direction
//       setSortDirection(sortDirection === 'asc' ? 'desc' : 'asc');
//     } else {
//       // Sinon, définir cette colonne comme colonne de tri et trier par ordre croissant par défaut
//       setSortColumn(column);
//       setSortDirection('asc'); // Ou 'desc' si vous préférez trier les nouvelles colonnes par défaut de manière descendante
//     }
//   };

//   /**
//    * Fonction de comparaison pour le tri des factures.
//    * Gère différents types de données (chaînes, nombres, dates).
...
//       if (valueA == null) return direction === 'asc' ? -1 : 1; // Nulls come first in asc, last in desc
//       if (valueB == null) return direction === 'asc' ? 1 : -1; // Nulls come last in asc, first in desc


//       let comparison = 0;

//       // Logique de comparaison basée sur le type de données
...
//           // Tenter de comparer comme des dates si le nom de colonne l'indique (ex: date_facture, date_soumission, last_modified_timestamp)
//            try {
...
//       }
//        else {
//           // Comparaison par défaut (convertir en chaîne)
//            comparison = String(valueA).toLowerCase().localeCompare(String(valueB).toLowerCase());
//       }
...
//   // Utilise useMemo si les performances deviennent un problème avec de très grandes listes
...
//   // -----------------------------------
//    // ... (handleDelete, handleUpdateStatus - le code de ces fonctions reste le même)
//     /**
//    * Gère le clic sur le bouton de suppression d'une facture.
//    * Inclut une vérification de rôle UI et une confirmation avant d'appeler onDelete.
//    * @param {number} id - ID de la facture à supprimer.
...
//       * Inclut une vérification de rôle UI avant d'appeler onUpdate.
//       * @param {number} factureId - ID de la facture à modifier.
//       * @param {string} currentStatut - Statut actuel de la facture ('soumis', 'approuve', 'rejete', 'paye').
//       */
//       const handleUpdateStatus = (factureId, currentStatut) => {
//            if (userRole !== 'gestionnaire' && userRole !== 'approbateur') {
//                 alert("Vous n'avez pas le rôle nécessaire pour changer le statut.");
...
//    * @param {string} column - La colonne actuelle.
//    * @returns {JSX.Element|null} Les flèches de tri ou null.
//    */
//    const renderSortArrow = (column) => {
//        if (sortColumn === column) {
//            return sortDirection === 'asc' ? ' ↑' : ' ↓';
//        }
//        return null; // Ou ' ↕' si vous voulez une indication de tri possible
//    };
...
//         <thead>
//           <tr className="bg-gray-100">
//             {/* En-têtes de colonne cliquables pour le tri */}
//             {/* Utilisez le nom de la propriété de l'objet facture comme identifiant de colonne */}
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('id')}>
//                  # {renderSortArrow('id')}
//             </th>
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('numero_facture')}>
//                 Numéro {renderSortArrow('numero_facture')}
//             </th>
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('date_facture')}>
//                 Date Facture {renderSortArrow('date_facture')}
//             </th>
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('type')}>
//                  Type {renderSortArrow('type')}
//             </th>
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('ubr')}>
//                  UBR {renderSortArrow('ubr')}
//             </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('fournisseur')}>
//                  Fournisseur {renderSortArrow('fournisseur')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('description')}>
//                  Description {renderSortArrow('description')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('montant')}>
//                  Montant {renderSortArrow('montant')}
//              </th>
//             <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('devise')}>
//                  Devise {renderSortArrow('devise')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('statut')}>
//                  Statut {renderSortArrow('statut')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('categorie')}>
//                  Catégorie {renderSortArrow('categorie')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('ligne_budgetaire')}>
//                  Ligne Budgétaire {renderSortArrow('ligne_budgetaire')}
//              </th>
//               <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('soumetteur_username')}>
//                  Soumetteur {renderSortArrow('soumetteur_username')}
//              </th>
//               <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('date_soumission')}>
//                  Date Soumission {renderSortArrow('date_soumission')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('created_by_username')}>
//                  Créé par {renderSortArrow('created_by_username')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('last_modified_by_username')}>
//                  Modifié par {renderSortArrow('last_modified_by_username')}
//              </th>
//              <th className="p-2 border cursor-pointer hover:bg-gray-200" onClick={() => handleHeaderClick('last_modified_timestamp')}>
//                  Dernière modification {renderSortArrow('last_modified_timestamp')}
//              </th>
//              <th className="p-2 border">Fichier</th> {/* Le fichier n'est généralement pas triable */}
//             {/* Afficher la colonne Actions uniquement si le rôle permet la suppression ou la mise à jour */}
//              {(userRole === 'gestionnaire' || userRole === 'approbateur') && (
//                 <th className="p-2 border">Actions</th>
//             )}
...
//         </thead>
//         <tbody>
//           {/* Utiliser sortedFactures au lieu de factures ici */}
...
//                   <td className="p-2 border">{facture.type}</td>
//                   <td className="p-2 border">{facture.ubr || 'N/A'}</td>
...
//                           {(userRole === 'gestionnaire' || userRole === 'approbateur') && (
//                              <button
//                                onClick={() => handleUpdateStatus(facture.id, facture.statut)}
//                                className="text-blue-500 hover:text-blue-700 text-sm"
//                              >
//                                {facture.statut === 'Soumis' ? 'Approuver/Rejeter/Payer' : 'Modifier Statut'}
//                              </button>
//                          )}
//                            {/* Bouton Modifier (si vous avez un formulaire/modale d'édition) */}
//                            {/*
//                            {(userRole === 'gestionnaire' || userRole === 'soumetteur') && ( // Exemple: soumetteur peut modifier SA facture
//                                 <button
//                                   onClick={() => onEditClick(facture)} // Appelle la fonction locale pour éditer
...
//       {/* Vue en cartes pour les écrans mobiles - Rendu conditionnel des actions */}
//       {/* Le tri affecte également l'ordre d'affichage des cartes car elles itèrent sur sortedFactures */}
//       <div className="sm:hidden">
//         {sortedFactures.length > 0 ? (
//             sortedFactures.map((facture) => (
//               <div key={facture.id} className="bg-white p-4 mb-4 rounded-lg shadow border">
